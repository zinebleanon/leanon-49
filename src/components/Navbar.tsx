...
import { Home, HelpCircle, Tag, ShoppingBag, Inbox, Bell, Menu, Users, MessageCircle, User } from 'lucide-react';
            ...
            {navItems.map((item) => (
              <NavigationMenuItem key={item.name}>
                <Link
                  ...
                  className={cn(
                    navigationMenuTriggerStyle(),
                    isPathActive(item.path) ? "bg-primary/10 text-primary" : "text-foreground/70 hover:text-foreground"
                  )}
                  onClick={(e) => {
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button 
                className="flex items-center justify-center h-10 w-10 rounded-full bg-white shadow-sm hover:bg-primary/5 transition-colors"
            ...
            <DropdownMenuContent align="end" className="w-56">
              {userInfo ? (
                ...
                <DropdownMenuItem onSelect={() => setIsJoinModalOpen(true)}>
